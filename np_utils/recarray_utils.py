...
def np_groupby(keyarr, arr, *functions, **kwds):
    ...
    groups = [
        np.fromiter((f(arr[i]) for i in index_groups), dtype=None, count=len(keys))
        for f in functions
    ]
...
def np_groupby_full(keyarr, arr, *functions_result_dtypes, **kwds):
    ...
    keys, index_groups = get_index_groups(keyarr)
    results = [
        _group_transform(arr, index_groups, fun, result_dtype)
...
def fields_view(arr, fields, force_ordering=False):
    ...
    """
    if force_ordering:
        ordered_fields = [n for n in arr.dtype.names if n in fields]
...
def rec_groupby(a, keynames, *fun_fields_name):
    ...
    functions = [_outfielder(fun, fields) for fun, fields, name in fun_fields_name]
...
def rec_groupby_full(a, keynames, *fun_dtype_fields_name):
    ...
    keynames = list(keynames) if islistlike(keynames) else [keynames]
    keyarr = fields_view(a, keynames, force_ordering=True)
    functions_result_dtypes = [
        (_outfielder(fun, fields), dtype)
        for fun, dtype, fields, name in fun_dtype_fields_name
    ]
    names = [i[-1] for i in fun_dtype_fields_name]
    return np_groupby_full(
        keyarr, a, *functions_result_dtypes, names=(keynames + names)
    )
...
def find_first_occurrence(arr):
    ...
    return np.array(
        [
            ...
            for s in (
                a.tostring(),
            )  # with s as a.tostring()   <-- would be this if python supported it
...
def cat_recarrays_on_columns(arrays, column_names=(), limit=None):
    ...
    """
    column_arrays = [
        np.concatenate([arr[i][:limit] for arr in arrays]) for i in column_names
    ]
...
def cat_recarrays(*arrays):
    ...
    """
    common_names = set.intersection(*[set(a.dtype.names) for a in arrays])
    output_column_names = [i for i in arrays[0].dtype.names if i in common_names]
    dummy_output = cat_recarrays_on_columns(
        arrays, column_names=output_column_names, limit=0
    )
    ...
    for column_name in output_column_names:
        ret_arr[column_name] = np.concatenate([a[column_name] for a in arrays])
...
def _rec_inner_join_helper(keycols, arr_list):
    ...
    names_and_dtypes_list = [
        list(zip(names, dtypes)) for names, dtypes in zip(names_list, dtypes_list)
    ]
    _nd_dict = dict(zip(names_list[0], dtypes_list[0]))
    key_dtypes = [_nd_dict[name] for name in keycols]

    non_key_names_and_dtypes = [
        [(name, dt) for name, dt in name_dtype_list if name not in keycols]
...
def rec_inner_join(keycols, *arr_list):
    ...
    for keys in keys_list[1:]:
        keys_use = [k for k in keys_use if k in set(keys)]
    ...
    output_lengths = [
        np.prod([len(d[k]) for d in index_groups_dict_list]) for k in keys_use
    ]  # The length of each key group after joining
    ...
    for key, start, length in zip(keys_use, output_starts, output_lengths):
        ...
        values = [
            arr[d[key]] for arr, d in zip(filtered_arrays, index_groups_dict_list)
        ]
