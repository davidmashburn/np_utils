...
def NDRectangle(start, end):
    ...
    start, end = [
        f([start, end], axis=0)  # go from small to large in each dimension
...
def nd_gradient(shape, origin_val, stopvals):
    ...
    ortho_grads = [
        g * (stop - origin_val) / (s - 1) for s, stop, g in zip(shape, stopvals, grids)
    ]
...
def nd_radial_gradient(shape, offsets=None):
    if offsets is None:
        offsets = [0] * len(shape)
    grids = np.mgrid.__getitem__(list(map(slice, shape)))
    v = [(g + off + 0.5 - s / 2) ** 2 for s, off, g in zip(shape, offsets, grids)]
    return np.sqrt(np.sum(v, axis=0))


def interpolated_radial_gradient(
    shape, stops, values, offsets=None, outer_radius=None, clip=True
):
    ...
    """
    outer_radius = (
        np.sqrt(np.sum(np.square(shape))) / 2 if outer_radius is None else outer_radius
    )
...
def GetDirectionsOfSteepestSlope(p0, p1, p2):
    ...
    maxs = [
        (None if i.max() == 0 else np.where(i == i.max())[0][0].tolist())
...
def GetDirectionsOfSteepestSlope_BorderCheckingVersion(borderPts):
    ...
    for i in range(ndim):  # loop over all dimensions as the assumed scan dimension (i)
        ...
        for (
            pts
        ) in binnedPts.values():  # loop through the points in each bin (hyperplane)
            deltas = [
                minMaxDiff(
                    [p[j] for p in pts]
                )  # get the maximum delta in each dimension
                for j in range(ndim)
            ]
...
def _getMinMaxList(borderPts, iscan, iline):
    ...
    for i, p in enumerate(borderPtsSort):
        if borderPtsSort[i - 1][iscan] != borderPtsSort[i][iscan]:
            minMaxList.append(
                [p, p]
            )  # ensure there are always 2 points, even if they are the same
        else:
            minMaxList[-1][-1] = p
    return minMaxList
...
def GetTriangleBorderPoints(p0, p1, p2):
    ...
    allPts = [BresenhamFunction(pa, pb) for pa, pb in ((p0, p1), (p1, p2), (p2, p0))]
...
def _map_BresLines(point_pairs):
    ...
    return [p for start, end in point_pairs for p in BresenhamFunction(start, end)]
...
def BresenhamTriangle(p0, p1, p2, doubleScan=True):  # Generalization for triangle
    ...
    if p2 == None:
        return BresenhamFunction(
            p0, p1
        )  # In case the last argument is None just use a plane...
...
def sample_points_from_plane(p, projected_pts, iscan, iline):
    ...
    new_points = _rounder(p[0] + np.outer(s, pts_diff[0]) + np.outer(t, pts_diff[1]))
    return new_points


def _BresenhamTriangle_PlaneFormulaVersion(
    p0, p1, p2, iscan_iline=None
):  # Generalization for triangle
    """Bresenham N-D triangle rasterization
    ...
    iscan, iline, borderPts = _BresTriBase(
        p0, p1, p2, dss_rerun=False, iscan_iline=iscan_iline
    )
