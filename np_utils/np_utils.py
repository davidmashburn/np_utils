...
one = np.array(
    1
)  # a surprisingly useful little array; makes lists into arrays by simply one*[[1,2],[3,6],...]
...
def build_grid(center, steps, nsteps):
    ...
    * a number of steps in each dimension
    ...
    assert all_equal(
        map(len, (center, steps, nsteps))
    ), "All the arguments must have the same length!"
    return np.meshgrid(
        *[
            np.linspace(c - (n - 1.0) / 2 * s, c + (n - 1.0) / 2 * s, n)
...
def shapeShift(arr, newShape, offset=None, fillValue=0):
    ...
    oldStartEnd = np.transpose(
        [np.clip(i - offset, 0, oldShape) for i in [0, newShape]]
    )
    newStartEnd = np.transpose(
        [np.clip(i + offset, 0, newShape) for i in [0, oldShape]]
    )
...
def addBorder(arr, borderValue=0, borderThickness=1, axis=None):
    ...
    allOrOne = slice(None) if axis == None else (axis if axis >= 0 else arr.ndim + axis)
...
def _transpose_interleaved(arr):
    ...
    [1, 3, 5, 7, ..., 0, 2, 4, ...]"""
    return arr.transpose(*zipflat(range(1, arr.ndim, 2), range(0, arr.ndim, 2)))
...
def shape_divide(arr, scale, reduction="mean"):
    ...
    assert reduction in reduction_options, "reduction must be one of: " + " ".join(
        reduction_options
    )
    scale = coerce_to_target_length(scale, arr.ndim)
    assert all(
        [sh % sc == 0 for sh, sc in zip(arr.shape, scale)]
    ...
    return (
        np.mean(flat_subarrays, axis=0)
        if reduction == "mean"
        else np.median(flat_subarrays, axis=0)
        if reduction == "median"
        else flat_subarrays[0]
        if reduction == "first"
        else subarrays
        if reduction == "all"
        else None
...
def reshape_repeating(arr, new_shape):
    ...
    if new_size <= arr.size:
        ...
        f = s.flat[
            :new_size
        ]  # This final slicing op is the first time a new array is created... hmmm
...
def remove_duplicate_subarrays(arr):
    ...
    seen = set()
    return np.array(
        [i for i in arr for s in (i.tostring(),) if s not in seen and not seen.add(s)]
    )
...
def polyCentroid(points):
    ...
    xyPairs = np.transpose(
        [points, np.roll(points, -1, axis=0)]
    )  # in (x0,x1),(y0,y1) format
    c = np.cross(*xyPairs)
    xySum = np.sum(xyPairs, axis=2)  # (x0+x1),(y0+y1)
    xc, yc = np.sum(xySum * c, axis=1) / (3.0 * np.sum(c))
    return xc, yc
...
def interpolateSumBelowAbove(arr, floatIndex, axis=0):
    ...
    above = (
        indp1Plane
        + np.sum(arr[slicer + (slice(ind + 2, None),)], axis=axis)
        + indPlane * (1 - rem)
    )
    return below, above
...
def limitInteriorPointsInterpolating(l, numInteriorPoints):
    ...
    l = np.asarray(l)
    return [
        interpNumpy(l, ind) for ind in np.linspace(0, len(l) - 1, numInteriorPoints + 2)
    ]
...
def reverseLinearTransform(points, oldRefLine, mirrored=False):
    ...
    mir = -1 if mirrored else 1
    matT = [[dx, -dy * mir], [dy, dx * mir]]
    dx2dy2 = np.square(dx) + np.square(dy)
    return np.dot(points, matT) * 1.0 / dx2dy2
...
def reverse_broadcast(f):
    ...
    newf.__doc__ = "\n".join(
        ["Transpose the arguments before and after running the function f:", f.__doc__]
    )
    return newf
...
def _broadcast_arr_list(l, reverse=False):
    ...
    broadcast = reverse_broadcast(broadcast_arrays) if reverse else broadcast_arrays
...
def apply_at_depth(f, *args, **kwds):
    ...
    assert not (
        "depth" in kwds and "depths" in kwds
    ), 'You can pass either kwd "depth" or "depths" but not both!'
    depths = kwds.pop(
        "depths", kwds.pop("depth", 0)
    )  # Grab depths or depth, fall back to 0
    broadcast_results = kwds.pop("broadcast_results", False)
    depths = depths if hasattr(depths, "__len__") else [depths] * len(args)
    ...
    results = results if not broadcast_results else _broadcast_arr_list(results)
