...
def gaussian_pdf(x, mu=0, sig=1, use_coeff=True):
    ...
    coeff = 1 / sig / np.sqrt(2.0 * np.pi) if use_coeff else 1
    return coeff * np.exp(-np.square(x - mu) / (2.0 * np.square(sig)))
...
def multi_random_normal(means, stds, counts, shuffle=True, random_state=np.random):
    ...
    out = np.concatenate(
        [
            random_state.normal(mean, std, count)
...
def uniformSphericalNoise(*shape, **kwds):
    ...
    return random_state.rand(shape[0], 1) ** (1.0 / shape[-1]) * normNoise
...
def addNoise(points, scales=1, dist="gaussian", random_state=np.random):
    ...
    """
    fun = {
        "uniform": random_state.rand,
        "gaussian": random_state.randn,
        "uniformradial": lambda *x: uniformSphericalNoise(
            *x, random_state=random_state
        ),
        "uniformcentered": lambda *x: _uniformCenteredNoise(
            *x, random_state=random_state
        ),
    }[dist]
...
def uniform_distribution_overlap(m1, s1, m2, s2):
    ...
    return (
        np.clip(
            (
                np.min(np.broadcast_arrays(m1 + s1, m2 + s2), axis=0)
...
def Bhattacharyya_coefficient(mu1, sig1, mu2, sig2):
    ...
    sigma_ratio_factor = np.sqrt(2.0 * sig1 * sig2 / sum_sig_sqr)
    exponent = -np.square(mu1 - mu2) / sum_sig_sqr / 4.0
    BC = sigma_ratio_factor * np.exp(exponent)
    return BC


gaussian_similarity = (
    Bhattacharyya_coefficient  # an alias since Bhattacharyya is so hard to spell :)
)
